...
case class EvalErrorOld(message: String, override val location: Option[Location])
  extends PhotonError(message, location) {}

sealed abstract class InterpreterModeOld(val name: String) {
  val shouldTryToPartiallyEvaluate: Boolean
}

object InterpreterModeOld {
  case object PartialEvaluation extends InterpreterModeOld("partial") {
    override val shouldTryToPartiallyEvaluate: Boolean = true
  }

  case object CompileTime extends InterpreterModeOld("compile-time") {
    override val shouldTryToPartiallyEvaluate: Boolean = false
  }

  case object CompileTimeOrPartial extends InterpreterModeOld("compile-time-or-partial") {
    override val shouldTryToPartiallyEvaluate: Boolean = true
  }

  case object Runtime extends InterpreterModeOld("runtime") {
    override val shouldTryToPartiallyEvaluate: Boolean = false
  }
}

class InterpreterOld(val interpreterMode: InterpreterModeOld) {
...
//        )
      ...
      case Value.Lambda(Lambda(params, _, body, traits), location) =>
        if (mode.shouldTryToPartiallyEvaluate) {
          ...
          val evalBody = evaluate(body, location, evalScope, InterpreterModeOld.PartialEvaluation)
      ...
      case Value.Operation(Operation.Block(values), location) =>
        ...
        values.zipWithIndex.foreach { case (value, index) =>
          ...
          if (evalValue.isDynamic) {
            if (mode.shouldTryToPartiallyEvaluate) {
              evalMode = InterpreterModeOld.PartialEvaluation
            } else if (mode == InterpreterModeOld.Runtime) {
              // TODO: Test this
              throw EvalErrorOld(s"Could not evaluate $evalValue", location)
            }
          }
      ...
      case let @ Value.Operation(Operation.Let(name, value, block), location) =>
        ...
        if (mode == InterpreterModeOld.Runtime && evalValue.isDynamic) {
          throw EvalErrorOld(s"Could not evaluate $let, probably due to the value referencing itself", location)
        }

        scopeMap.addOne(name, evalValue)

        val blockEvalMode = if (mode.shouldTryToPartiallyEvaluate && evalValue.isDynamic) {
          InterpreterModeOld.PartialEvaluation
        } else {
          mode
        }
...
//      val context = CallContext()
      ...
      Core.nativeValueFor(evalTarget).method(context, name, location) match {
        case Some(method) =>
          if (canEvaluate(method.traits, mode)) {
            ...
            result
          } else if (mode == InterpreterModeOld.Runtime && !method.traits.contains(LambdaTrait.Runtime)) {
            throw EvalErrorOld(s"Could not evaluate compile-time-only function call $call", location)
          } else {
            logger.debug(s"Not evaluating $call because it does not have the required trait for $mode")
            call
          }

        case None => throw EvalErrorOld(s"Cannot call method $name on ${evalTarget.toString}", location)
      }
    } else {
      val partiallyEvaluatedTarget = if (mode.shouldTryToPartiallyEvaluate && shouldEvalTarget && targetWasLambda) {
        evalTarget match {
          case Value.Lambda(_, _) => evaluate(evalTarget, scope, InterpreterModeOld.PartialEvaluation)
          case _ => evalTarget
        }
      } else target
  ...
  private def evaluate(block: Operation.Block, location: Option[Location], scope: Scope, mode: InterpreterModeOld): Operation.Block = {
  ...
  private def canEvaluate(traits: Set[LambdaTrait], mode: InterpreterModeOld): Boolean = {
    mode match {
      case InterpreterModeOld.PartialEvaluation => traits.contains(LambdaTrait.Partial)
      case InterpreterModeOld.CompileTime => traits.contains(LambdaTrait.CompileTime)
      case InterpreterModeOld.Runtime => traits.contains(LambdaTrait.Runtime)

      // TODO: Should this switch to Partial if the function supports that instead?
      case InterpreterModeOld.CompileTimeOrPartial => traits.contains(LambdaTrait.CompileTime)
    }
  }

  private def evalError(value: Value, message: String): Nothing = {
    throw EvalErrorOld(s"Cannot evaluate ${value.inspectAST}. $message".strip(), value.location)
  }
}
